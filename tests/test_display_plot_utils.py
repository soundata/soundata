...
def test_on_slider_changed(mocker):
    # Mocking dependencies
    current_time = [0.0]
    current_time_lock = MockLock()  # Use the custom MockLock
    line1 = mocker.Mock()
    line2 = mocker.Mock()
    fig = mocker.Mock()
    fig.canvas = mocker.Mock()

    # Patch matplotlib figure
    ...
    # Calling the function with a mock 'change' object
    on_slider_changed({"new": 10.0})

    # Asserts to check if the function ran (still not checking logic)
    line1.set_xdata.assert_called_with([10.0, 10.0])
    line2.set_xdata.assert_called_with([10.0, 10.0])
    fig.canvas.draw_idle.assert_called_once()


def test_on_reset_clicked(mocker):
    # Mocking dependencies
    current_time = [0.0]
    current_time_lock = MockLock()
    line1 = mocker.Mock()
    line2 = mocker.Mock()
    slider = mocker.Mock()
    play_pause_button = mocker.Mock()
    fig = mocker.Mock()
    fig.canvas = mocker.Mock()

    play_thread = [mocker.Mock()]
    playing = [True]  # Assume audio is playing
    ...
    def on_reset_clicked(b):
        ...
        if playing[0]:
            mocker.patch("threading.Event.set")
    ...
    # Asserts to check if the function correctly reset the state
