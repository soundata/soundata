...
There are other libraries that handle datasets like `Tensorflow` [@tensorflow] or `Tensorflow Datasets` [@tensorflow_datasets], `DCASE-models` [@zinemanas2020dcase], and `HuggingFace Datasets` [@lhoest2021datasets]. But none of them serves as a stand-alone library that can easily be plugged into different work pipelines, with different modeling software.  Having a community-centric, open-source, audio-specialized library allows us greater flexibility to incorporate more audio-specific API functionalities and align our priorities with those of the audio community.
